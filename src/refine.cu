...
__device__ int findIncidentOri(int* d_trianglelist, int tri1, int tri2)
{
    if (tri1 < 0 || tri2 < 0)
        return -1;
    int inc0 = -1, inc1 = -1;
    int tri1_p[3] = {d_trianglelist[3 * tri1], d_trianglelist[3 * tri1 + 1],
                     d_trianglelist[3 * tri1 + 2]};
    int tri2_p[3] = {d_trianglelist[3 * tri2], d_trianglelist[3 * tri2 + 1],
...
__device__ bool checkseg4encroach(REAL2 vOrg,
                                  REAL2 vDest,
                                  REAL2 vApex,
                                  REAL  theta,
                                  int   run_mode)
    ...
    if (dotproduct < 0.0)  // angle > 90
    {
        ...
        if (run_mode || dotproduct * dotproduct >=
                            (2.0 * goodcoss - 1.0) * (2.0 * goodcoss - 1.0) *
                                ((vOrg.x - vApex.x) * (vOrg.x - vApex.x) +
                                 (vOrg.y - vApex.y) * (vOrg.y - vApex.y)) *
                                ((vDest.x - vApex.x) * (vDest.x - vApex.x) +
                                 (vDest.y - vApex.y) * (vDest.y - vApex.y)))
            return true;
    }
...
int updateEmptyPoints(PStatusD& t_PStatus, IntD& t_emptypoints)
{
    ...
    t_emptypoints.erase(thrust::copy_if(first, last, t_PStatus.begin(),
                                        t_emptypoints.begin(), isDeleted()),
                        t_emptypoints.end());

    return t_emptypoints.size();
}

int updateEmptyTriangles(TStatusD& t_TStatus, IntD& t_emptytriangles)
{
    ...
    t_emptytriangles.erase(thrust::copy_if(first, last, t_TStatus.begin(),
                                           t_emptytriangles.begin(), isEmpty()),
                           t_emptytriangles.end());

    return t_emptytriangles.size();
}

int updateActiveListByMarker_Slot(IntD& t_marker,
                                  IntD& t_active,
                                  int   numberofelements)
{
    ...
    t_active.erase(thrust::copy_if(first, last, t_marker.begin(),
                                   t_active.begin(), isNotNegativeInt()),
                   t_active.end());

    return t_active.size();
}

int updateActiveListByMarker_Val(IntD& t_marker,
                                 IntD& t_active,
                                 int   numberofelements)
{
    t_active.resize(numberofelements);
    ...
    t_active.erase(
        thrust::copy_if(t_marker.begin(), t_marker.begin() + numberofelements,
                        t_active.begin(), isNotNegativeInt()),
        t_active.end());
...
int updateActiveListByFlipMarker(TStatusD& t_TStatus,
                                 IntD&     t_active,
                                 int       numberoftriangles)
{
    ...
    t_active.erase(thrust::copy_if(first, last, t_TStatus.begin(),
                                   t_active.begin(), isFlipFlop()),
                   t_active.end());

    return t_active.size();
}

int updateActiveListByFlipWinner(TStatusD& t_TStatus,
                                 IntD&     t_active,
                                 int       numberoftriangles,
                                 int       minsize)
{
    ...
    t_active.erase(thrust::copy_if(first, last, t_TStatus.begin(),
                                   t_active.begin(), isFlipWinner()),
                   t_active.end());

    return t_active.size();
}

int updateActiveListByFlipNegate(IntD& t_flipBy,
                                 IntD& t_active,
                                 int   numberoftriangles,
                                 int   minsize)
{
    ...
    t_active.erase(thrust::copy_if(first, last, t_flipBy.begin(),
                                   t_active.begin(), isNegativeInt()),
                   t_active.end());

    return t_active.size();
}

__device__ REAL kernelComputeSmallestAngle(REAL2 pApex,
                                           REAL2 pOrg,
                                           REAL2 pDest,
                                           int*  longest,
                                           REAL* shortestedge,
...
__global__ void kernelMarkBadTriangles(REAL2*   d_pointlist,
                                       PStatus* d_PStatus,
                                       int*     d_trianglelist,
                                       int*     d_neighborlist,
                                       int*     d_segmentlist,
                                       ...
                                       REAL     theta)
{
    ...
    int p[3] = {d_trianglelist[3 * index], d_trianglelist[3 * index + 1],
                d_trianglelist[3 * index + 2]};

    REAL2 v[3] = {d_pointlist[p[0]], d_pointlist[p[1]], d_pointlist[p[2]]};

    int  longest;
    int  shortest;
    REAL shortestedge;
    REAL smallestCos2 = kernelComputeSmallestAngle(v[0], v[1], v[2], &longest,
    ...
    } else  // has bad angle
    {
        ...
        if (d_tri2subseg[3 * index + shortest] == -1) {
            ...
            if (d_PStatus[eOrg].isSegmentSplit() &&
                d_PStatus[eDest].isSegmentSplit()) {
                int s1 = -1, s2 = -1;  // record two segments
                int pOrg1, pDest1, pOrg2, pDest2,
                    common;  // record the segment endpoints that include the
                             // shortest edge endpoints
                int curTri, curOri;
                ...
                if (common != -1) {
                    ...
                    dist1 = (vCom.x - vOrg.x) * (vCom.x - vOrg.x) +
                            (vCom.y - vOrg.y) * (vCom.y - vOrg.y);
                    dist2 = (vCom.x - vDest.x) * (vCom.x - vDest.x) +
                            (vCom.y - vDest.y) * (vCom.y - vDest.y);
                    // if equal
...
int updateActiveListToBadTriangles(Real2D&   t_pointlist,
                                   PStatusD& t_PStatus,
                                   IntD&     t_trianglelist,
                                   IntD&     t_neighborlist,
                                   IntD&     t_segmentlist,
                                   ...
                                   REAL      theta)
{
    ...
    t_active0.erase(thrust::copy_if(first0, last0, t_TStatus.begin(),
                                    t_active0.begin(), isCheckOrBadTriangle()),
                    t_active0.end());
    ...
    kernelMarkBadTriangles<<<numberofblocks, BLOCK_SIZE>>>(
        ...
        thrust::raw_pointer_cast(&t_active0[0]), numberofcheck, theta);
    ...
    t_active1.erase(thrust::copy_if(first1, last1, t_TStatus.begin(),
                                    t_active1.begin(), isBadTriangle()),
                    t_active1.end());
...
__device__ void markFan(int*     d_trianglelist,
                        int*     d_neighborlist,
                        TStatus* d_TStatus,
                        int      triIndex,
                        int      triOri)
...
__global__ void kernelMarkReduantPoints(REAL2*   d_pointlist,
                                        PStatus* d_PStatus,
                                        int*     d_trianglelist,
                                        int*     d_neighborlist,
                                        TStatus* d_TStatus,
                                        ...
                                        int      numberofdels)
{
    ...
    if (d_PStatus[pApex].isSteiner() && !d_PStatus[pApex].isSegmentSplit() &&
        ((vOrg.x - vApex.x) * (vDest.x - vApex.x) +
             (vOrg.y - vApex.y) * (vDest.y - vApex.y) <
         0.0)) {
        ...
        markFan(d_trianglelist, d_neighborlist, d_TStatus, triIndex,
                (triOri + 2) % 3);
    ...
    if (otri != -1) {
        ...
        // free steiner points and inside the diametral circle
        if (d_PStatus[pApex].isSteiner() &&
            !d_PStatus[pApex].isSegmentSplit() &&
            ((vOrg.x - vApex.x) * (vDest.x - vApex.x) +
                 (vOrg.y - vApex.y) * (vDest.y - vApex.y) <
             0.0)) {
            // mark this point as reduntant
...
void markReduntantPoints(Real2D&   t_pointlist,
                         PStatusD& t_PStatus,
                         IntD&     t_trianglelist,
                         IntD&     t_neighborlist,
                         TStatusD& t_TStatus,
                         ...
                         int       numberofdels)
{
    ...
    kernelMarkReduantPoints<<<numberofblocks, BLOCK_SIZE>>>(
        ...
        thrust::raw_pointer_cast(&t_dellist[0]), numberofdels);
}
...
__device__ void atomic_compete_two(int      index0,
                                   int      index1,
                                   int      ori,
                                   int*     d_flipBy,
                                   TStatus* d_TStatus)
...
__device__ void atomic_compete_three(int      index0,
                                     int      index1,
                                     int      index2,
                                     int      ori,
                                     int*     d_flipBy,
...
__device__ void atomic_compete_four(int      index0,
                                    int      index1,
                                    int      index2,
                                    int      index3,
                                    int      ori,
...
__global__ void kernelInitFlipOri(TStatus* d_TStatus, int numberoftriangle)
{
    int pos = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    d_TStatus[pos].setFlipOri(
        15);  // indicate they are losers or undefined (at the beginning)
}

__global__ void kernelCheckFlipping(REAL2*   d_pointlist,
                                    PStatus* d_PStatus,
                                    int*     d_trianglelist,
                                    int*     d_neighborlist,
                                    int*     d_tri2subseg,
                                    ...
                                    int      numberofactive)
{
    ...
    int p[3] = {d_trianglelist[index * 3 + 0], d_trianglelist[index * 3 + 1],
    ...
    const REAL2 v[3] = {d_pointlist[p[0]], d_pointlist[p[1]],
                        d_pointlist[p[2]]};
    ...
    for (int i = 0; i < 3; i++)  // three edges
    {
        if (min == p[i])
            continue;
        if (d_tri2subseg[3 * index + i] != -1)
            continue;  // segment cannot flip
        ...
        if (index < pTri ||
            (pTri >= 0 &&
             !d_TStatus[pTri].isFlip()))  // index < pTri or pTri is unmarked
        {
            ...
            if (min != MAXINT)  // Involve a point to be deleted
            {
                ...
                {
                    atomic_compete_three(index, pTri, decode_tri(n1), deleteOri,
                                         d_flipBy, d_TStatus);
                    return;
                } else  // degree >= 4
                {
                    ...
                    if (decode_tri(n3) ==
                        decode_tri(n2))  // 4-degree reduntant point
                    {
                        ...
                        if (min == p[org]) {
                            ...
                            if (d == 0) {
                                ...
                                if (!f0 &&
                                    !f1)  // both are unflippable, do 4-1 flip
                                {
                                    if (d_PStatus[n1_pOpp].isReduntant() &&
                                        n1_pOpp <
                                            min)  // gurantees the smallest
                                                  // index reduntant point
                                        continue;
                                    deleteOri = i;
                                    atomic_compete_four(index, pTri, n1_tri,
                                                        n2_tri, deleteOri,
                                                        d_flipBy, d_TStatus);
                                    return;
                                }
                        ...
                        } else  // inward direction
                        {
                            ...
                            if (d == 0) {
                                ...
                                if (!f0 &&
                                    !f1)  // both are unflippable, do 4-2 flip
                                {
                                    if (d_PStatus[n1_pOpp].isReduntant() &&
                                        n1_pOpp <
                                            min)  // gurantees the smallest
            ...
            REAL ret_after =
                cuda_inCircle(v[apex], v[org], vOpp, v[dest]);  // after flip
            ...
            if (ret_after_opp == 0)
                ret_after_opp =
                    cuda_inCircle_exact(vOpp, v[dest], v[apex], v[org]);

            if (ret > 0 && ret_opp > 0 && ret_after < 0 &&
                ret_after_opp < 0)  // prevent calculation error
            {
...
__global__ void kernelMarkFlipWinner(PStatus* d_PStatus,
                                     int*     d_trianglelist,
                                     int*     d_neighborlist,
                                     TStatus* d_TStatus,
                                     int*     d_flipBy,
                                     ...
                                     int      numberofactive)
{
    ...
    if (d_flipBy[index] == index) {
        if (ori < 3)  // 2->2 flip
        {
            ...
            if (d_flipBy[pOppTri] ==
                index) {  // I'm also the one who win the right
                isWinner = true;
            }
        ...
        } else  // 4->2 flip
        {
            ...
            if (d_PStatus[pOrg].isReduntant() &&  // when pOrg is the smallest
                                                  // redundant point
                (!d_PStatus[pDest].isReduntant() ||
                 (d_PStatus[pDest].isReduntant() && pOrg < pDest))) {
            ...
            if (d_flipBy[pOppTri] == index && d_flipBy[t1] == index &&
                d_flipBy[t2] == index) {
                isWinner = true;
            }
        }
...
__global__ void kernelInitLinklist(PStatus* d_PStatus,
                                   int*     d_trianglelist,
                                   int*     d_neighborlist,
                                   TStatus* d_TStatus,
                                   int*     d_active,
                                   ...
                                   int      step)
{
    ...
    } else  // 4-2 flip
    {
        ...
        int p[] = {d_trianglelist[3 * pOppTri], d_trianglelist[3 * pOppTri + 1],
...
__device__ void setTriangle(int* d_trianglelist,
                            int  pOrg,
                            int  pDest,
                            int  pApex,
                            int  tri,
...
__device__ void setNeighborlink(int* d_linkslot,
                                int  nOrg,
                                int  nDest,
                                int  nApex,
                                int  listIndex,
...
__global__ void kernelUpdatePhase1(PStatus* d_PStatus,
                                   int*     d_trianglelist,
                                   int*     d_neighborlist,
                                   int*     d_tri2subseg,
                                   TStatus* d_TStatus,
                                   ...
                                   int      step)
{
    ...
    int pOpp, pOppTri, pOppOri, nOpp, pOrg, pDest, pApex, nOrg, nDest, nApex,
        npDest, npApex;
    ...
    if (ori < 3)  // 2->2 flip
    {
        ...
        setNeighborlink(d_linkslot, MAXINT, npDest + 3, npApex + 3, 4 * pos + 1,
                        pOppOri);
    ...
    } else if (ori < 6)  // 3->1 flip
    {
        ...
        setNeighborlink(d_linkslot, nOrg + 3, nDest + 3, nApex + 3, 4 * pos,
                        ori);
    ...
    } else                            // 4->2 flip
    {
        ...
        if (d_PStatus[pOrg]
                .isReduntant() &&  // when pOrg is the smallest redundant point
            (!d_PStatus[pDest].isReduntant() ||
             (d_PStatus[pDest].isReduntant() && pOrg < pDest))) {
            t1 = d_neighborlist[index * 3 + (ori + 2) % 3];
            t2 = d_neighborlist[pOppTri * 3 + (pOppOri + 1) % 3];
        ...
        if (d_PStatus[pOrg]
            ...
            setNeighborlink(d_linkslot, nOrg + 3, nDest + 3, MAXINT, 4 * pos,
                            ori);
            setNeighborlink(d_linkslot, MAXINT, npDest + 3, npApex + 3,
                            4 * pos + 2, t1_ori);
        ...
        } else  // inward direction
        {
            ...
            setNeighborlink(d_linkslot, nOrg + 3, MAXINT, nApex + 3, 4 * pos,
                            ori);
            setNeighborlink(d_linkslot, MAXINT, npDest + 3, npApex + 3,
                            4 * pos + 2, t1_ori);
...
__device__ void updateNeighborlink(int* d_linklist,
                                   int* d_linkslot,
                                   int  wIndex,
                                   int  otri,
                                   int  neighbor,
...
__global__ void kernelUpdatePhase2(PStatus* d_PStatus,
                                   int*     d_trianglelist,
                                   int*     d_neighborlist,
                                   int*     d_tri2subseg,
                                   TStatus* d_TStatus,
                                   ...
                                   int      iteration)
{
    ...
    int ori, pOrg, pDest, pOpp, pOppTri, pOppOri, nOrg, nDest, nApex, npDest,
        npApex;
    int nOrgOri, nDestOri, nApexOri, npDestOri, npApexOri;
    ...
    if (ori < 3)  // 2->2 flip
    {
        ...
        if (tempIndex != -1) {
            ...
            if (winnerIndex != MAXINT && winnerIndex >= 0 &&
                ...
                updateNeighborlink(d_linklist, d_linkslot, winnerIndex, nOrg,
                                   encode_tri(index, ori), numberofactive);
        }
        ...
        if (tempIndex != -1) {
            ...
            if (winnerIndex != MAXINT && winnerIndex >= 0 &&
                d_TStatus[winnerIndex].getFlipOri() != 15)
                updateNeighborlink(d_linklist, d_linkslot, winnerIndex, nApex,
                                   encode_tri(pOppTri, pOppOri),
                                   numberofactive);
        }
        ...
        if (tempIndex != -1) {
            ...
            if (winnerIndex != MAXINT && winnerIndex >= 0 &&
                d_TStatus[winnerIndex].getFlipOri() != 15)
                updateNeighborlink(d_linklist, d_linkslot, winnerIndex, npDest,
                                   encode_tri(index, (ori + 1) % 3),
                                   numberofactive);
        }
        ...
        if (tempIndex != -1) {
            ...
            if (winnerIndex != MAXINT && winnerIndex >= 0 &&
                d_TStatus[winnerIndex].getFlipOri() != 15)
                updateNeighborlink(d_linklist, d_linkslot, winnerIndex, npApex,
                                   encode_tri(pOppTri, (pOppOri + 2) % 3),
                                   numberofactive);
        }
    ...
    } else if (ori < 6)  // 3->1 flip
    {
        ...
        if (tempIndex != -1) {
            ...
            if (winnerIndex != MAXINT && winnerIndex >= 0 &&
                d_TStatus[winnerIndex].getFlipOri() != 15)
                updateNeighborlink(d_linklist, d_linkslot, winnerIndex, nDest,
                                   encode_tri(index, (ori + 1) % 3),
                                   numberofactive);
        }
    ...
    } else  // 4-2 flip
    {
        ...
        if (outward) {
            ...
            if (tempIndex != -1) {
                ...
                if (winnerIndex != MAXINT && winnerIndex >= 0 &&
                    d_TStatus[winnerIndex].getFlipOri() != 15)
                    updateNeighborlink(d_linklist, d_linkslot, winnerIndex,
                                       nOrg, encode_tri(index, ori),
                                       numberofactive);
            }
            ...
            if (tempIndex != -1) {
                ...
                if (winnerIndex != MAXINT && winnerIndex >= 0 &&
                    d_TStatus[winnerIndex].getFlipOri() != 15)
                    updateNeighborlink(d_linklist, d_linkslot, winnerIndex,
                                       nDest, encode_tri(index, (ori + 1) % 3),
                                       numberofactive);
            }
            ...
            if (tempIndex != -1) {
                ...
                if (winnerIndex != MAXINT && winnerIndex >= 0 &&
                    d_TStatus[winnerIndex].getFlipOri() != 15)
                    updateNeighborlink(d_linklist, d_linkslot, winnerIndex,
                                       npDest, encode_tri(t1, (t1_ori + 1) % 3),
                                       numberofactive);
            }
            ...
            if (tempIndex != -1) {
                ...
                if (winnerIndex != MAXINT && winnerIndex >= 0 &&
                    d_TStatus[winnerIndex].getFlipOri() != 15)
                    updateNeighborlink(d_linklist, d_linkslot, winnerIndex,
                                       npApex, encode_tri(t1, (t1_ori + 2) % 3),
                                       numberofactive);
            }
        ...
        } else  // inward direction
        {
            ...
            if (tempIndex != -1) {
                ...
                if (winnerIndex != MAXINT && winnerIndex >= 0 &&
                    d_TStatus[winnerIndex].getFlipOri() != 15)
                    updateNeighborlink(d_linklist, d_linkslot, winnerIndex,
                                       nApex, encode_tri(index, (ori + 2) % 3),
                                       numberofactive);
            }
...
__device__ void resetEncseg(int*   d_trianglelist,
                            REAL2* d_pointlist,
                            int*   d_encmarker,
                            int    segIndex,
                            int    otri0,
...
__global__ void kernelUpdatePhase3(REAL2*   d_pointlist,
                                   PStatus* d_PStatus,
                                   int*     d_trianglelist,
                                   int*     d_neighborlist,
                                   int*     d_tri2subseg,
                                   ...
                                   int      last_triangle)
{
    ...
    if (ori < 3)  // 2->2 flip
    {
        ...
        if (nApex > 0) {
            nApex = -(nApex - 3);
            updateNeighborlist(d_neighborlist, -nApex,
                               encode_tri(pOppTri, pOppOri));
        }

        if (npDest > 0) {
            npDest = -(npDest - 3);
            updateNeighborlist(d_neighborlist, -npDest,
                               encode_tri(index, (ori + 1) % 3));
        }

        if (npApex > 0) {
            npApex = -(npApex - 3);
            updateNeighborlist(d_neighborlist, -npApex,
                               encode_tri(pOppTri, (pOppOri + 2) % 3));
        }
        ...
        d_neighborlist[index * 3 + (ori + 2) % 3] =
            encode_tri(pOppTri, (pOppOri + 1) % 3);
        d_neighborlist[pOppTri * 3 + pOppOri] = -nApex;
        d_neighborlist[pOppTri * 3 + (pOppOri + 1) % 3] =
            encode_tri(index, (ori + 2) % 3);
        ...
        if (sOrg != -1) {
            otri0 = (index << 2) | ori;
            otri1 = -nOrg;
            resetEncseg(d_trianglelist, d_pointlist, d_encmarker, sOrg, otri0,
                        otri1, theta, run_mode);
        }

        if (sDest != -1) {
            otri0 = (index << 2) | ((ori + 1) % 3);
            otri1 = -npDest;
            resetEncseg(d_trianglelist, d_pointlist, d_encmarker, sDest, otri0,
                        otri1, theta, run_mode);
        }
        ...
        if (spOrg != -1) {
            ...
            otri1 = -nApex;
            resetEncseg(d_trianglelist, d_pointlist, d_encmarker, spOrg, otri0,
                        otri1, theta, run_mode);
        }
        ...
        if (spApex != -1) {
            ...
            otri1 = -npApex;
            resetEncseg(d_trianglelist, d_pointlist, d_encmarker, spApex, otri0,
                        otri1, theta, run_mode);
        }
    ...
    } else if (ori < 6)  // 3-1 flip
    {
        ...
        if (nDest > 0) {
            nDest = -(nDest - 3);
            updateNeighborlist(d_neighborlist, -nDest,
                               encode_tri(index, (ori + 1) % 3));
        }
        ...
        if (sApex != -1) {
            ...
            ;
            otri1 = -nApex;
            resetEncseg(d_trianglelist, d_pointlist, d_encmarker, sApex, otri0,
                        otri1, theta, run_mode);
        }
    ...
    } else  // 4-2 flip
    {
        ...
        if (outward)  // outward direction
        {
            ...
            if (nOrg > 0) {  // My neighbor do not update me, update him
                nOrg = -(nOrg - 3);
                updateNeighborlist(d_neighborlist, -nOrg,
                                   encode_tri(index, ori));
            }
            ...
            if (npDest > 0) {
                npDest = -(npDest - 3);
                updateNeighborlist(d_neighborlist, -npDest,
                                   encode_tri(t1, (t1_ori + 1) % 3));
            }

            if (npApex > 0) {
                npApex = -(npApex - 3);
                updateNeighborlist(d_neighborlist, -npApex,
                                   encode_tri(t1, (t1_ori + 2) % 3));
            }
            ...
            if (sOrg != -1) {
                otri0 = (index << 2) | ori;
                otri1 = -nOrg;
                resetEncseg(d_trianglelist, d_pointlist, d_encmarker, sOrg,
                            otri0, otri1, theta, run_mode);
            }

            if (sDest != -1) {
                otri0 = (index << 2) | ((ori + 1) % 3);
                otri1 = -nDest;
                resetEncseg(d_trianglelist, d_pointlist, d_encmarker, sDest,
                            otri0, otri1, theta, run_mode);
            }
            ...
            if (spDest != -1) {
                ...
                otri1 = -npDest;
                resetEncseg(d_trianglelist, d_pointlist, d_encmarker, spDest,
                            otri0, otri1, theta, run_mode);
            }
            ...
            if (spApex != -1) {
                ...
                otri1 = -npApex;
                resetEncseg(d_trianglelist, d_pointlist, d_encmarker, spApex,
                            otri0, otri1, theta, run_mode);
            }
            ...
            if (sApex != -1) {
                otri0 = (index << 2) | ((ori + 2) % 3);
                otri1 = -nApex;
                resetEncseg(d_trianglelist, d_pointlist, d_encmarker, sApex,
                            otri0, otri1, theta, run_mode);
            }
...
__global__ void kernelMarkReduntantSteiner(PStatus* d_PStatus,
                                           int*     d_trianglelist,
                                           int*     d_neighborlist,
                                           TStatus* d_TStatus,
                                           int*     d_flipBy,
...
void flipFlop(Real2D&   t_pointlist,
              PStatusD& t_PStatus,
              IntD&     t_trianglelist,
              IntD&     t_neighborlist,
              IntD&     t_tri2subseg,
              ...
              int       iteration1)
{
    ...
    do {
        ...
        numberofactive = updateActiveListByFlipMarker(t_TStatus, t_active,
                                                      numberoftriangles);
        ...
        kernelInitFlipOri<<<numberofblocks, BLOCK_SIZE>>>(
            thrust::raw_pointer_cast(&t_TStatus[0]), numberoftriangles);
        ...
        kernelCheckFlipping<<<numberofblocks, BLOCK_SIZE>>>(
            ...
            thrust::raw_pointer_cast(&t_active[0]), numberofactive);
        ...
        numberofwinner = updateActiveListByFlipWinner(
            t_TStatus, t_active, numberoftriangles, numberofactive);
        ...
        kernelInitLinklist<<<numberofblocks, BLOCK_SIZE>>>(
            ...
            thrust::raw_pointer_cast(&t_linklist[0]), numberofwinner,
            thrust::raw_pointer_cast(&t_tri2subseg[0]), iteration0, iteration1,
            step);
        ...
        kernelUpdatePhase1<<<numberofblocks, BLOCK_SIZE>>>(
            ...
            thrust::raw_pointer_cast(&t_flipBy[0]),
            thrust::raw_pointer_cast(&t_active[0]),
            thrust::raw_pointer_cast(&t_linkslot[0]), numberofwinner,
            iteration0, iteration1, step);
        ...
        kernelUpdatePhase3<<<numberofblocks, BLOCK_SIZE>>>(
            ...
            thrust::raw_pointer_cast(&t_linklist[0]),
            thrust::raw_pointer_cast(&t_linkslot[0]), numberofwinner, run_mode,
            theta, iteration0, step, numberoftriangles);

        // mark reduntant steiners and incident triangles
        numberofnegate = updateActiveListByFlipNegate(
            t_flipBy, t_active, numberoftriangles, numberofactive);
        ...
        kernelMarkReduntantSteiner<<<numberofblocks, BLOCK_SIZE>>>(
            ...
            thrust::raw_pointer_cast(&t_TStatus[0]),
            thrust::raw_pointer_cast(&t_flipBy[0]),
            thrust::raw_pointer_cast(&t_active[0]), numberofnegate);
...
__global__ void kernelResetMidInsertionMarker(int*     d_neighborlist,
                                              TStatus* d_TStatus,
                                              int*     d_subseg2tri,
                                              int*     d_trimarker,
...
__global__ void kernelMarkMidInsertion(int*     d_neighborlist,
                                       TStatus* d_TStatus,
                                       int*     d_subseg2tri,
                                       int*     d_trimarker,
                                       int*     d_internalmarker,
...
__device__ void insertTriangle(int* d_trianglelist,
                               int* d_neighborlist,
                               int  v0,
                               int  v1,
                               int  v2,
...
__global__ void kernelInsertMidPoints(REAL2*   d_pointlist,
                                      PStatus* d_PStatus,
                                      int*     d_trianglelist,
                                      int*     d_neighborlist,
                                      int*     d_tri2subseg,
                                      ...
                                      int      iteration)
{
    ...
    newID_point =
        d_emptypoints[emptypointsLength - numberofemptypoints + tindex];
    newID_tri1 = d_emptytriangles[emptytrianglesLength -
                                  numberofemptytriangles + 2 * tindex];
    newID_tri2 = d_emptytriangles[emptytrianglesLength -
                                  numberofemptytriangles + 2 * tindex + 1];
    ...
    REAL segmentlength = sqrt((d_pointlist[p[0]].x - d_pointlist[p[1]].x) *
                                  (d_pointlist[p[0]].x - d_pointlist[p[1]].x) +
                              (d_pointlist[p[0]].y - d_pointlist[p[1]].y) *
                                  (d_pointlist[p[0]].y - d_pointlist[p[1]].y));
    ...
    d_pointlist[newID_point] =
        MAKE_REAL2(d_pointlist[p[0]].x +
                       (d_pointlist[p[1]].x - d_pointlist[p[0]].x) * split,
                   d_pointlist[p[0]].y +
                       (d_pointlist[p[1]].y - d_pointlist[p[0]].y) * split);

    REAL multiplier = cuda_ccw(d_pointlist[p[0]], d_pointlist[p[1]],
                               d_pointlist[newID_point]);
    REAL divisor = ((d_pointlist[p[0]].x - d_pointlist[p[1]].x) *
                        (d_pointlist[p[0]].x - d_pointlist[p[1]].x) +
                    (d_pointlist[p[0]].y - d_pointlist[p[1]].y) *
                        (d_pointlist[p[0]].y - d_pointlist[p[1]].y));
    ...
    if ((multiplier != 0.0) && (divisor != 0.0)) {
        ...
        if (multiplier == multiplier) {
            d_pointlist[newID_point].x +=
                multiplier * (d_pointlist[p[1]].y - d_pointlist[p[0]].y);
            d_pointlist[newID_point].y +=
                multiplier * (d_pointlist[p[0]].x - d_pointlist[p[1]].x);
        }
    }
    ...
    // debug
    if (d_pointlist[p[0]].x == d_pointlist[p[1]].x &&
        d_pointlist[p[0]].y == d_pointlist[p[1]].y) {
        ...
        printf(
            "Thread %d: Segment %d - %d (Steiner: %d, Mid: %d), %d (Steiner: "
            "%d, Mid: %d)\n",
            pos, seg[0], p[0], d_PStatus[p[0]].isSteiner() ? 1 : 0,
            d_PStatus[p[0]].isSegmentSplit() ? 1 : 0, p[1],
            d_PStatus[p[1]].isSteiner() ? 1 : 0,
            d_PStatus[p[1]].isSegmentSplit() ? 1 : 0);
        printf(
            "Thread %d: Duplicate midpoint, endpoint %d(%.19lf,%.19lf) and "
            "%d(%.19lf,%.19lf), midpoint %d(%.19lf,%.19lf)\n",
            pos, p[0], d_pointlist[p[0]].x, d_pointlist[p[0]].y, p[1],
            d_pointlist[p[1]].x, d_pointlist[p[1]].y, newID_point,
            d_pointlist[newID_point].x, d_pointlist[newID_point].y);
        int pseg = d_subseg2seg[seg[0]];
        int pe1, pe2;
        pe1 = d_segmentlist[2 * pseg];
        pe2 = d_segmentlist[2 * pseg + 1];
        printf(
            "Thread %d: Parent segment %d - %d(%.19f,%.19f), %d(%.19lf,%.19f), "
            "boundary %d\n",
            pos, pseg, pe1, d_pointlist[pe1].x, d_pointlist[pe1].y, pe2,
            d_pointlist[pe2].x, d_pointlist[pe2].y, (tri[2] != -1) ? 0 : 1);
        int num_split = 0;
        ...
        for (int i = 0; i < 4121; i++) {
            ...
            if (incident) {
                ...
                REAL cosa = (ev[1].x - ev[0].x) * (ev[2].x - ev[0].x) +
                            (ev[1].y - ev[0].y) * (ev[2].y - ev[0].y);
                if (cosa <= 0)
                    printf(
                        "Thread %d: Incident Segment %d - %d, %d, angle >= 90 "
                        "degree\n",
                        pos, i, ep1, ep2);
                else {
                    cosa *= cosa;
                    cosa /= ((ev[1].x - ev[0].x) * (ev[1].x - ev[0].x) +
                             (ev[1].y - ev[0].y) * (ev[1].y - ev[0].y)) *
                            ((ev[2].x - ev[0].x) * (ev[2].x - ev[0].x) +
                             (ev[2].y - ev[0].y) * (ev[2].y - ev[0].y));
                    if (cosa > cos2) {
                        printf(
                            "Thread %d: Incident Segment %d - %d, %d, < 15 "
                            "degree\n",
                            pos, i, ep1, ep2);
                    } else if (cosa > cos1) {
                        printf(
                            "Thread %d: Incident Segment %d - %d, %d, 15 "
                            "degree <= angle < 60 degree\n",
                            pos, i, ep1, ep2);
                    } else {
                        printf(
                            "Thread %d: Incident Segment %d - %d, %d, 60 "
                            "degree <= angle < 90 degree\n",
                            pos, i, ep1, ep2);
                    }
                }
            }
    ...
    d_subseg2seg[numberofsubseg + tindex] =
        d_subseg2seg[seg[0]];  // copy my parent to you
    if (seg[1] != -1)
...
__global__ void kernelUpdateMidNeighbors(REAL2*   d_pointlist,
                                         int*     d_trianglelist,
                                         int*     d_neighborlist,
                                         int*     d_tri2subseg,
                                         TStatus* d_TStatus,
                                         ...
                                         REAL     theta)
{
    ...
    for (int i = 0; i < iteration; i++)  // for each pTri[i]
    {
        ...
        if (pOppTri < 0)  // dont need to update neighbor, but need to update
                          // encroach marker
        {
            if (seg != -1) {
                d_encmarker[seg] = -1;
                ...
                if (neighbor > -1) {
                    ...
                    if (p[0] == p[3] && p[1] == p[2]) {
                        ...
                        if (seg != -1) {
                            ...
                            if (checkseg4encroach(v[0], v[1], v[2], theta,
                                                  run_mode))
                                d_encmarker[seg] = 0;
                            else if (checkseg4encroach(v[1], v[0], v[3], theta,
                                                       run_mode))
                                d_encmarker[seg] = 0;
                        }
...
__global__ void kernelUpdatePStatus2Old(PStatus* d_PStatus, int last_point)
{
    int pos = blockIdx.x * blockDim.x + threadIdx.x;
...
void updatePStatus2Old(PStatusD& t_PStatus, int last_point)
{
    int numberofblocks = (ceil)((float)last_point / BLOCK_SIZE);
    kernelUpdatePStatus2Old<<<numberofblocks, BLOCK_SIZE>>>(
        thrust::raw_pointer_cast(&t_PStatus[0]), last_point);
}
...
__global__ void kernelMarkAllEncsegs(REAL2* d_pointlist,
                                     int*   d_trianglelist,
                                     int*   d_neighborlist,
                                     int*   d_subseg2tri,
                                     int*   d_encmarker,
...
void markAllEncsegs(Real2D& t_pointlist,
                    IntD&   t_trianglelist,
                    IntD&   t_neighborlist,
                    IntD&   t_subseg2tri,
                    IntD&   t_encmarker,
                    ...
                    REAL    theta)
{
    ...
    kernelMarkAllEncsegs<<<numberofblocks, BLOCK_SIZE>>>(
        ...
        thrust::raw_pointer_cast(&t_neighborlist[0]),
        thrust::raw_pointer_cast(&t_subseg2tri[0]),
        thrust::raw_pointer_cast(&t_encmarker[0]), numberofsubseg, run_mode,
        theta);
}

void splitEncsegs(Real2D&   t_pointlist,
                  PStatusD& t_PStatus,
                  IntD&     t_trianglelist,
                  IntD&     t_neighborlist,
                  IntD&     t_tri2subseg,
                  ...
                  int       debug_iter)
{
    ...
    while (true) {
        ...
        numberofencs = updateActiveListByMarker_Slot(t_encmarker, t_enclist,
                                                     *numberofsubseg);
        ...
        if (false) {
            ...
            cudaMemcpy(debug_el, thrust::raw_pointer_cast(&t_enclist[0]),
                       sizeof(int) * numberofencs, cudaMemcpyDeviceToHost);
        ...
        thrust::copy(t_enclist.begin(), t_enclist.end(),
                     t_internalmarker.begin());

        // delete all points inside diametral circle
        int step = 0;
        while (true) {
            // update deletion subsegs active list and marker
            numberofdels = updateActiveListByMarker_Val(
                t_internalmarker, t_internallist, t_internalmarker.size());
            ...
            markReduntantPoints(t_pointlist, t_PStatus, t_trianglelist,
                                t_neighborlist, t_TStatus, t_subseg2tri,
                                t_internalmarker, t_internallist, numberofdels);

            // maintain denauly property, do flip-flop
            flipFlop(t_pointlist, t_PStatus, t_trianglelist, t_neighborlist,
                     t_tri2subseg, t_TStatus, t_subseg2tri, t_flipBy,
                     t_flipActive, t_encmarker, t_linklist, t_linkslot,
                     *numberoftriangles, run_mode, theta, -1, -1);

            // check if encroachment markers are updated correctly
            if (false) {
                // printf("Iteration %d, Step %d: After Remove redundant
                // points\n",iteration,step);
                ...
                cudaMemcpy(debug_em, thrust::raw_pointer_cast(&t_encmarker[0]),
                           sizeof(int) * (*numberofsubseg),
                           cudaMemcpyDeviceToHost);
                cudaMemcpy(debug_tl,
                           thrust::raw_pointer_cast(&t_trianglelist[0]),
                           sizeof(int) * 3 * *numberoftriangles,
                           cudaMemcpyDeviceToHost);
                cudaMemcpy(debug_nl,
                           thrust::raw_pointer_cast(&t_neighborlist[0]),
                           sizeof(int) * 3 * *numberoftriangles,
                           cudaMemcpyDeviceToHost);
                cudaMemcpy(debug_pl, thrust::raw_pointer_cast(&t_pointlist[0]),
                           sizeof(REAL2) * *numberofpoints,
                           cudaMemcpyDeviceToHost);
                cudaMemcpy(debug_st, thrust::raw_pointer_cast(&t_subseg2tri[0]),
                           sizeof(int) * *numberofsubseg,
                           cudaMemcpyDeviceToHost);
                ...
                for (int i = 0; i < *numberofsubseg; i++) {
                    ...
                    v[2] = debug_pl[p[2]];
                    bool tag =
                        false;  // indicate if this segment is encroached or not
                    ...
                    if (dotproduct < 0.0)  // angle > 90
                    {
                        ...
                        if (run_mode ||
                            dotproduct * dotproduct >=
                                (2.0 * goodcoss - 1.0) *
                                    (2.0 * goodcoss - 1.0) *
                                    ((v[0].x - v[2].x) * (v[0].x - v[2].x) +
                                     (v[0].y - v[2].y) * (v[0].y - v[2].y)) *
                    ...
                    if (otri != -1) {
                        ...
                        if (dotproduct < 0.0)  // angle > 90
                        {
                            // here, we use diametral lens to speedup the
                            // algorithm
                            if (run_mode ||
                                dotproduct * dotproduct >=
                                    (2.0 * goodcoss - 1.0) *
                    ...
                    if (debug_em[i] == -1 && tag)
                        printf(
                            "Line 3356, iteration = %d, step = %d, Segment %d: "
                            "I am encroached but marked as non-encroached\n",
                            iteration, step, i);

                    if (debug_em[i] == 0 && !tag)
                        printf(
                            "Line 3359, iteration = %d, step = %d, Segment %d: "
                            "I am not encroached but marked as encroached\n",
                            iteration, step, i);

                    // if( debug_em[i] == 1)
                    //	printf("Line 3362, iteration = %d, step = %d, Segment
                    //%d: I am marked as encroached because I am on
                    //segment\n",iteration,step,i);
                }
        ...
        if (false) {
            ...
            cudaMemcpy(debug_tl, thrust::raw_pointer_cast(&t_trianglelist[0]),
                       sizeof(int) * 3 * *numberoftriangles,
                       cudaMemcpyDeviceToHost);
            cudaMemcpy(debug_pl, thrust::raw_pointer_cast(&t_pointlist[0]),
                       sizeof(REAL2) * *numberofpoints, cudaMemcpyDeviceToHost);
            cudaMemcpy(debug_ts, thrust::raw_pointer_cast(&t_TStatus[0]),
                       sizeof(TStatus) * *numberoftriangles,
                       cudaMemcpyDeviceToHost);
            for (int i = 0; i < *numberoftriangles; i++) {
                if (!debug_ts[i].isNull()) {
                    ...
                    if (errorflag)
                        printf(
                            "After remove redundant points - Tri %d: Duplicate "
                            "vertice\n",
                            i);
                }
            }
        ...
        if (2 * numberofencs > *numberofemptytriangles) {
            *numberofemptytriangles =
                updateEmptyTriangles(t_TStatus, t_emptytriangles);
            int num = 0;
        ...
        // use internal marker and list for insertion subsegs, use t_flipBy as
        // insertion marker init insertion subseg marker
        ...
        while (true) {
            // inside one triangle, more than one segment may split, violation
            // may happen

            // update insertion subsegs active list and marker
            // t_internallist store the indices for t_enclist
            // in order to keep thread id for kernels, do not resize
            // t_internalmarker
            numberofmids = updateActiveListByMarker_Slot(
                t_internalmarker, t_internallist, numberofencs);
            ...
            kernelResetMidInsertionMarker<<<numberofblocks, BLOCK_SIZE>>>(
                ...
                thrust::raw_pointer_cast(&t_internallist[0]), numberofmids);
            ...
            kernelMarkMidInsertion<<<numberofblocks, BLOCK_SIZE>>>(
                ...
                thrust::raw_pointer_cast(&t_internallist[0]), numberofmids,
                thrust::raw_pointer_cast(&t_trianglelist[0]),
                thrust::raw_pointer_cast(&t_tri2subseg[0]), debug_iter);
            ...
            kernelInsertMidPoints<<<numberofblocks, BLOCK_SIZE>>>(
                ...
                t_emptypoints.size(), t_emptytriangles.size(),
                *numberofemptypoints, *numberofemptytriangles, *numberofsubseg,
                numberofmids, run_mode, theta, iteration);
        ...
        *numberofsubseg += numberofencs;
        ...
        if (false) {
            ...
            int*   debug_st = new int[*numberofsubseg];
            cudaMemcpy(debug_em, thrust::raw_pointer_cast(&t_encmarker[0]),
                       sizeof(int) * (*numberofsubseg), cudaMemcpyDeviceToHost);
            cudaMemcpy(debug_tl, thrust::raw_pointer_cast(&t_trianglelist[0]),
                       sizeof(int) * 3 * *numberoftriangles,
                       cudaMemcpyDeviceToHost);
            cudaMemcpy(debug_nl, thrust::raw_pointer_cast(&t_neighborlist[0]),
                       sizeof(int) * 3 * *numberoftriangles,
                       cudaMemcpyDeviceToHost);
            cudaMemcpy(debug_pl, thrust::raw_pointer_cast(&t_pointlist[0]),
                       sizeof(REAL2) * *numberofpoints, cudaMemcpyDeviceToHost);
            ...
            for (int i = 0; i < *numberofsubseg; i++) {
                ...
                if (debug_em[i] == -1 && tag)
                    printf(
                        "Line 3628, iteration = %d, Segment %d: I am "
                        "encroached but marked as non-encroached\n",
                        i, iteration);

                if (debug_em[i] == 0 && !tag)
                    printf(
                        "Line 3631, iteration = %d, Segment %d: I am not "
                        "encroached but marked as encroached\n",
                        i, iteration);
            }
            // printf("Finish Checking\n");
        }
        ...
        if (false) {
            ...
            for (int i = 0; i < *numberoftriangles; i++) {
                if (!debug_ts[i].isNull()) {
                    ...
                    if (errorflag)
                        printf(
                            "After insert midpoints - Tri %d (%d, %d, %d): "
                            "Duplicate vertice\n",
                            i, p[0], p[1], p[2]);
                }
            }
        ...
        // maintain denauly property, do flip-flop
        flipFlop(t_pointlist, t_PStatus, t_trianglelist, t_neighborlist,
                 t_tri2subseg, t_TStatus, t_subseg2tri, t_flipBy, t_flipActive,
                 t_encmarker, t_linklist, t_linkslot, *numberoftriangles,
                 run_mode, theta, -1, -1);

        // check if encroachment markers are updated correctly
        if (false) {
            // printf("Iteration %d: After Insert midpoints and
            // flipFlop\n",iteration);
            int*   debug_em = new int[*numberofsubseg];
            int*   debug_tl = new int[3 * (*numberoftriangles)];
            int*   debug_nl = new int[3 * (*numberoftriangles)];
            ...
            for (int i = 0; i < *numberofsubseg; i++) {
                ...
                if (debug_em[i] == -1 && tag)
                    printf(
                        "Line 3760, iteration = %d, Segment %d: I am "
                        "encroached but marked as non-encroached\n",
                        i, iteration);

                if (debug_em[i] == 0 && !tag)
                    printf(
                        "Line 3763, iteration = %d, Segment %d: I am not "
                        "encroached but marked as encroached\n",
                        i, iteration);
            }
            // printf("Finish Checking\n");
...
__device__ REAL2 computeCircumcenter(REAL2 vApex,
                                     REAL2 vOrg,
                                     REAL2 vDest,
                                     REAL  offConstant,
                                     bool  offcenter,
                                     REAL* priority)
{
    ...
    dadist = (vDest.x - vApex.x) * (vDest.x - vApex.x) +
             (vDest.y - vApex.y) * (vDest.y - vApex.y);
    ...
    if (tmp != 0)
        ...
        denominator =
            0.5 / cuda_ccw(vDest, vApex,
                           vOrg);  // use ccw () to ensure a positive (and
                                   // reasonably accurate) result, avoiding any
    ...
    if (offcenter &&
        (offConstant > 0.0)) /* Find the position of the off-center, as
                                described by Alper Ungor. */
    {
        if ((dodist < aodist) && (dodist < dadist)) {
            ...
            dxoff =
                0.5 * (vApex.x - vDest.x) - offConstant * (vApex.y - vDest.y);
            dyoff =
                0.5 * (vApex.y - vDest.y) + offConstant * (vApex.x - vDest.x);
...
__global__ void kernelComputeCircumcenter(REAL2* d_pointlist,
                                          int*   d_trianglelist,
                                          int*   d_active,
                                          int    numberofactive,
                                          REAL2* d_TCenter,
                                          ...
                                          bool   offCenter)
{
    ...
    // center coordinate
    REAL  priority;
    REAL2 cc = computeCircumcenter(v[0], v[1], v[2], offConstant, offCenter,
                                   &priority);
...
__global__ void kernelLocateSinkPoints(REAL2*   d_pointlist,
                                       PStatus* d_PStatus,
                                       int*     d_trianglelist,
                                       int*     d_neighborlist,
                                       int*     d_tri2subseg,
                                       ...
                                       int      numberofactive)
{
    ...
    if (!split &&
        direction_dart != 3)  // need to check neighbors of bad triangles
    {
    ...
    while (checkNeighbor)  // trave from bad triangle to circumcenter
    {
        ...
        //{
        //	printf("Tri %d: My circumcenter encountered duplicate vertex:
        //%d(%lf,%lf), Steiner: %d, Midpoint: %d\n", 		index,
        ...
        if (detLeft < 0.0) {
            if (detRight < 0.0) {
                ...
                moveleft = (v[(curOri + 2) % 3].x - v[(curOri + 1) % 3].x) *
                                   (cc.x - v[curOri].x) +
                               (v[(curOri + 2) % 3].y - v[(curOri + 1) % 3].y) *
                                   (cc.y - v[curOri].y) >
                           0.0;
            } else {
                moveleft = true;
        ...
        // circumcenter is out of boundary, mark current trianlge as segement
        // split or circumcenter is on the opposite side of segment
    ...
    {
        int neighbor = d_neighborlist[3 * curTri + direction_dart];
        if (neighbor != -1)  // prevent that steiner point happen to be on or
                             // out of the boundary
        {
...
__global__ void kernelRemoveLosers(int*     d_trianglelist,
                                   int*     d_neighborlist,
                                   TStatus* d_TStatus,
                                   int*     d_trimarker,
                                   int*     d_sinks,
...
__global__ void kernelMarkCavities(REAL2*   d_pointlist,
                                   int*     d_trianglelist,
                                   int*     d_neighborlist,
                                   int*     d_tri2subseg,
                                   TStatus* d_TStatus,
                                   ...
                                   int      numofprocess)
{
    ...
    if (d_internalmarker[pos + offset] <
        0)  // lost because of locating competition
        return;
    ...
    while (true) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            {
                if (direction == 3 &&
                    d_tri2subseg[3 * curIndex + i] !=
                        -1)  // no segment split, don't need to check
                    continue;

                if (direction < 3 && d_tri2subseg[3 * curIndex + i] != -1 &&
                    i != direction)  // if segment split, check that segment
                    continue;
            } else if (d_tri2subseg[3 * curIndex + i] !=
                       -1)  // has segment edge: no need to check
                continue;
            ...
            if (region.find(
                    neighborIndex))  // searched already: no need to check
                continue;
...
void markCavities(Real2D&   t_pointlist,
                  IntD&     t_trianglelist,
                  IntD&     t_neighborlist,
                  IntD&     t_tri2subseg,
                  TStatusD& t_TStatus,
...
)
{
    ...
    do {
        ...
        kernelMarkCavities<<<numberofblocks, BLOCK_SIZE>>>(
            ...
            thrust::raw_pointer_cast(&t_internalmarker[0]),
            thrust::raw_pointer_cast(&t_internallist[0]), numberofbad,
...
__global__ void kernelCheckCavities(REAL2*   d_pointlist,
                                    int*     d_trianglelist,
                                    int*     d_neighborlist,
                                    int*     d_tri2subseg,
                                    TStatus* d_TStatus,
                                    ...
                                    int      numofprocess)
{
    ...
    int index = d_sinks[pos + offset] >>
                2;  // use the index for bad triangle to mark cavity
    int direction = d_sinks[pos + offset] & 3;
    int priority = d_Priority[pos + offset];
...
void checkCavities(Real2D&   t_pointlist,
                   IntD&     t_trianglelist,
                   IntD&     t_neighborlist,
                   IntD&     t_tri2subseg,
                   TStatusD& t_TStatus,
...
__global__ void kernelResetSteinerInsertionMarker(int*     d_neighborlist,
                                                  TStatus* d_TStatus,
                                                  int*     d_internallist,
                                                  int*     d_sinks,
                                                  int      numberofactive)
...
__global__ void kernelInsertSteinerPoints(REAL2*   d_pointlist,
                                          PStatus* d_PStatus,
                                          int*     d_trianglelist,
                                          int*     d_neighborlist,
                                          int*     d_tri2subseg,
                                          ...
                                          int      iteration)
{
    ...
    if (direction != 3 &&
        d_tri2subseg[3 * index + direction] != -1)  // steiner is on the segment
    {
        // printf("Return ");
        return;  // mark later
    }
    ...
    newID_tri1 = d_emptytriangles[emptytrianglesLength -
                                  numberofemptytriangles + 2 * pos];
    newID_tri2 = d_emptytriangles[emptytrianglesLength -
                                  numberofemptytriangles + 2 * pos + 1];
    tri[0] = index;
    ...
    } else  // steiner is on the edge
    {
        ...
        for (int i = 0; i < 3; i++)
            seg[i] = d_tri2subseg[3 * tri[0] + (triOri[0] + i) % 3];
        d_tri2subseg[3 * tri[0]] = seg[2];  // old segment
        d_tri2subseg[3 * tri[0] + 1] =
            seg[0];  // old segment | new edge (should be -1)
        d_tri2subseg[3 * tri[0] + 2] = -1;  // new edge
        d_tri2subseg[3 * tri[1]] = seg[1];  // old segment
        d_tri2subseg[3 * tri[1] + 1] = -1;  // new edge
        d_tri2subseg[3 * tri[1] + 2] =
            seg[0];  // old segment | new edge (should be -1)
        ...
        neighbor[2] = d_neighborlist[tri[0] * 3 + (triOri[0] + 2) % 3];
        if (tri[2] == -1) {
            insertTriangle(d_trianglelist, d_neighborlist, newID_point, p[2],
                           p[0], tri[0], neighbor[2], -1, tri[1] << 2 | 1);
            insertTriangle(d_trianglelist, d_neighborlist, newID_point, p[1],
                           p[2], tri[1], neighbor[1], tri[0] << 2 | 2, -1);
        } else {
            insertTriangle(d_trianglelist, d_neighborlist, newID_point, p[2],
                           p[0], tri[0], neighbor[2], tri[2] << 2 | 2,
                           tri[1] << 2 | 1);
            insertTriangle(d_trianglelist, d_neighborlist, newID_point, p[1],
                           p[2], tri[1], neighbor[1], tri[0] << 2 | 2,
                           tri[3] << 2 | 1);
        }
        ...
        if (tri[2] != -1) {
            ...
            d_tri2subseg[3 * tri[2]] = seg[1];  // old segment
            d_tri2subseg[3 * tri[2] + 1] = -1;  // new edge
            d_tri2subseg[3 * tri[2] + 2] =
                seg[0];  // old segment | new edge (should be -1)
            d_tri2subseg[3 * tri[3]] = seg[2];  // old segment
            d_tri2subseg[3 * tri[3] + 1] =
                seg[0];  // old segment | new edge (should be -1)
            d_tri2subseg[3 * tri[3] + 2] = -1;
            ...
            neighbor[2] = d_neighborlist[tri[2] * 3 + (triOri[1] + 2) % 3];
            insertTriangle(d_trianglelist, d_neighborlist, newID_point, p[1],
                           p[2], tri[2], neighbor[1], tri[3] << 2 | 2,
                           tri[0] << 2 | 1);
            insertTriangle(d_trianglelist, d_neighborlist, newID_point, p[2],
                           p[0], tri[3], neighbor[2], tri[1] << 2 | 2,
                           tri[2] << 2 | 1);
        }
    }
}

__global__ void kernelUpdateSteinerNeighbors(REAL2*   d_pointlist,
                                             int*     d_trianglelist,
                                             int*     d_neighborlist,
                                             int*     d_tri2subseg,
                                             TStatus* d_TStatus,
                                             ...
                                             int      debug_iter)
{
    ...
    if (direction != 3 &&
        d_TStatus[index].getFlipOri() == 15)  // steiner is on the segment
        return;                               // mark later
...
__global__ void kernelMarkSteinerOnsegs(int* d_tri2subseg,
                                        int* d_enclist,
                                        int* d_internallist,
                                        int* d_sinks,
                                        int  numberofactive)
...
__global__ void kernelUpdateSteinerOnsegs(int* d_encmarker,
                                          int* d_internallist,
                                          int  numberofactive)
{
    int pos = blockIdx.x * blockDim.x + threadIdx.x;
...
void splitTriangles(Real2D&   t_pointlist,
                    PStatusD& t_PStatus,
                    IntD&     t_trianglelist,
                    IntD&     t_neighborlist,
                    IntD&     t_tri2subseg,
                    ...
                    int       debug_iter)
{
    ...
    thrust::copy(t_internalmarker.begin(), t_internalmarker.end(),
                 t_internallist.begin());
    ...
    kernelComputeCircumcenter<<<numberofblocks, BLOCK_SIZE>>>(
        ...
        thrust::raw_pointer_cast(&t_internallist[0]), numberofbad,
        thrust::raw_pointer_cast(&t_TCenter[0]),
        thrust::raw_pointer_cast(&t_Priority[0]), offconstant, offcenter);

    // Locate sinks
    // Use t_trimaker as marker for locating
    thrust::fill(t_trimarker.begin(), t_trimarker.begin() + *numberoftriangles,
                 MAXINT);
    ...
    // numberofwinner =
    // updateActiveListByMarker_Slot(t_internalmarker,t_internallist,numberofbad);
    // printf("numberofwinner = %d\n",numberofwinner);

    // Independent Steiner points
    // (1). Mark cavities: use trimarker as marker
    // (2). Check cavities and remove dependent points (excluding incident
    // cavities, which will be removed in flip-flop stage)
    thrust::fill(t_trimarker.begin(), t_trimarker.begin() + *numberoftriangles,
                 MAXINT);

    markCavities(t_pointlist, t_trianglelist, t_neighborlist, t_tri2subseg,
                 t_TStatus, t_TCenter, t_sinks, t_Priority, t_trimarker,
                 t_internalmarker, t_internallist, numberofbad, 2500);

    checkCavities(t_pointlist, t_trianglelist, t_neighborlist, t_tri2subseg,
                  t_TStatus, t_TCenter, t_sinks, t_Priority, t_trimarker,
                  t_internalmarker, t_internallist, numberofbad, 2500);

    numberofsteiner = updateActiveListByMarker_Slot(
        t_internalmarker, t_internallist, numberofbad);
    ...
    kernelMarkSteinerOnsegs<<<numberofblocks, BLOCK_SIZE>>>(
        ...
        thrust::raw_pointer_cast(&t_enclist[0]),
        thrust::raw_pointer_cast(&t_internallist[0]),
        thrust::raw_pointer_cast(&t_sinks[0]), numberofsteiner);
    ...
    kernelInsertSteinerPoints<<<numberofblocks, BLOCK_SIZE>>>(
        ...
        thrust::raw_pointer_cast(&t_sinks[0]),
        thrust::raw_pointer_cast(&t_emptypoints[0]),
        thrust::raw_pointer_cast(&t_emptytriangles[0]), t_emptypoints.size(),
        t_emptytriangles.size(), *numberofemptypoints, *numberofemptytriangles,
        *numberofsubseg, numberofsteiner, theta, debug_iter);
    ...
    kernelUpdateSteinerNeighbors<<<numberofblocks, BLOCK_SIZE>>>(
        ...
        thrust::raw_pointer_cast(&t_internalmarker[0]),
        thrust::raw_pointer_cast(&t_internallist[0]),
        thrust::raw_pointer_cast(&t_sinks[0]), numberofsteiner, run_mode, theta,
        debug_iter);
    ...
    *numberofemptytriangles -= 2 * numberofsteiner;
    ...
    if (false) {
        ...
        for (int i = 0; i < *numberofsubseg; i++) {
            ...
            if (debug_em[i] == -1 && tag)
                printf(
                    "Line 5382, Segment %d: I am encroached but marked as "
                    "non-encroached\n",
                    i);

            if (debug_em[i] == 0 && !tag)
                printf(
                    "Line 5385, Segment %d: I am not encroached but marked as "
                    "encroached\n",
                    i);
        }
        // printf("Finish Checking\n");
    ...
    // maintain denauly property, do flip-flop
    flipFlop(t_pointlist, t_PStatus, t_trianglelist, t_neighborlist,
             t_tri2subseg, t_TStatus, t_subseg2tri,
             t_trimarker,  // flipBy
             t_sinks,      // flipActive
             t_encmarker,
    ...
    if (false) {
        ...
        for (int i = 0; i < *numberofsubseg; i++) {
            ...
            if (debug_em[i] == -1 && tag)
                printf(
                    "Line 5491, Segment %d: I am encroached but marked as "
                    "non-encroached\n",
                    i);

            if (debug_em[i] == 0 && !tag)
                printf(
                    "Line 5494, Segment %d: I am not encroached but marked as "
                    "encroached\n",
                    i);
        }
        // printf("Finish Checking\n");
    ...
    numberofonsegs = updateActiveListByMarker_Slot(t_enclist, t_internallist,
                                                   *numberofsubseg);
    ...
    kernelUpdateSteinerOnsegs<<<numberofblocks, BLOCK_SIZE>>>(
        thrust::raw_pointer_cast(&t_encmarker[0]),
        thrust::raw_pointer_cast(&t_internallist[0]), numberofonsegs);
}
...
__global__ void kernelUpdateNeighborsFormat2Otri(int* d_trianglelist,
                                                 int* d_neighborlist,
                                                 int  numberofprocess,
                                                 int  poffset)
{
    ...
    if (pos >= numberofprocess)
        return;
    int neighbors[3] = {d_neighborlist[3 * (pos + poffset)],
    ...
    for (int i = 0; i < 3; i++)
        d_neighborlist[3 * (pos + poffset) + i] =
            encode_tri(neighbors[i], neighborsOri[i]);
}

void updateNeighborsFormat2Otri(IntD& t_trianglelist,
                                IntD& t_neighborlist,
                                int   numberoftriangles,
                                int   maxnumofblock)
{
    ...
    do {
        ...
        kernelUpdateNeighborsFormat2Otri<<<numberofblocks, BLOCK_SIZE>>>(
            thrust::raw_pointer_cast(&t_trianglelist[0]),
            thrust::raw_pointer_cast(&t_neighborlist[0]), numberofprocess,
            finishnum);
...
__global__ void kernelInitSubsegs(int* d_segmentlist,
                                  int* d_subseg2tri,
                                  int* d_trianglelist,
                                  int* d_tri2subseg,
                                  int  numberoftriangles,
                                  ...
                                  int  numoftri)
{
    ...
    for (int i = scanoffset; i < scanoffset + numofscan; i++) {
        if (threadId + i * BLOCK_SIZE < numberofsegments) {
            ...
            s[2 * threadId + 1] =
                d_segmentlist[2 * (threadId + i * BLOCK_SIZE) + 1];
        }
        __syncthreads();  // wait for all threads in the same block finish
                          // reading
        ...
        if (pos < numoftri) {
            for (int j = 0; j < BLOCK_SIZE; j++) {
                ...
                if (sindex < numberofsegments) {
                    ...
                    if ((p[0] == index0 && p[1] == index1) ||
                        (p[0] == index1 && p[1] == index0))  // ori 2
                    {
                        int otri = (pos << 2) | 2;
                        atomicMax(d_subseg2tri + sindex, otri);  // larger index
                        d_tri2subseg[3 * pos + 2] = sindex;
                    } else if ((p[1] == index0 && p[2] == index1) ||
                               (p[1] == index1 && p[2] == index0))  // ori 0
                    {
                        int otri = (pos << 2) | 0;
                        atomicMax(d_subseg2tri + sindex, otri);
                        d_tri2subseg[3 * pos + 0] = sindex;
                    } else if ((p[2] == index0 && p[0] == index1) ||
                               (p[2] == index1 && p[0] == index0))  // ori 1
                    {
                        int otri = (pos << 2) | 1;
                        atomicMax(d_subseg2tri + sindex, otri);
...
void initSubsegs(IntD& t_segmentlist,
                 IntD& t_subseg2tri,
                 IntD& t_trianglelist,
                 IntD& t_tri2subseg,
                 int   numberoftriangles,
...
{
    int numofscan = (ceil)((float)numberofsegments /
                           BLOCK_SIZE);  // number of scans totally
    ...
    do {
        ...
        do {
            ...
            kernelInitSubsegs<<<numberofblocks, BLOCK_SIZE>>>(
                ...
                thrust::raw_pointer_cast(&t_subseg2tri[0]),
                thrust::raw_pointer_cast(&t_trianglelist[0]),
                thrust::raw_pointer_cast(&t_tri2subseg[0]), numberoftriangles,
                numberofsegments, scanfinish, scanprocess, trifinish,
                triprocess);
...
__global__ void kernelMarkValidTriangles_compact(TStatus* d_TStatus,
                                                 int*     d_valid,
                                                 int      numberofprocess,
                                                 int      poffset)
{
...
__global__ void kernelCollectTrianglesEmptySlots_compact(TStatus* d_TStatus,
                                                         int*     d_prefix,
                                                         int*     d_empty,
                                                         int numberofprocess,
                                                         int poffset)
...
__global__ void kernelFillTrianglesEmptySlots_compact(TStatus* d_TStatus,
                                                      int*     d_prefix,
                                                      int*     d_empty,
                                                      int*     d_trianglelist,
                                                      int*     d_neighborlist,
                                                      ...
                                                      int offset)
{
    ...
    if (pos + poffset < numberofelements_new)
        ...
        for (int i = 0; i < 3; i++) {
            d_trianglelist[value * 3 + i] =
                d_trianglelist[(pos + poffset) * 3 + i];
            d_neighborlist[value * 3 + i] =
                d_neighborlist[(pos + poffset) * 3 + i];
        }
    }
...
__global__ void kernelFixNeighbors_compact(int* d_neighborlist,
                                           int* d_newIndex,
                                           int  numberofprocess,
                                           int  poffset)
{
    ...
    if (d_neighborlist[3 * triIndex] >= 0)
        d_neighborlist[3 * triIndex] =
            encode_tri(d_newIndex[decode_tri(d_neighborlist[3 * triIndex])],
                       decode_ori(d_neighborlist[3 * triIndex]));

    if (d_neighborlist[3 * triIndex + 1] >= 0)
        d_neighborlist[3 * triIndex + 1] =
            encode_tri(d_newIndex[decode_tri(d_neighborlist[3 * triIndex + 1])],
                       decode_ori(d_neighborlist[3 * triIndex + 1]));

    if (d_neighborlist[3 * triIndex + 2] >= 0)
        d_neighborlist[3 * triIndex + 2] =
            encode_tri(d_newIndex[decode_tri(d_neighborlist[3 * triIndex + 2])],
                       decode_ori(d_neighborlist[3 * triIndex + 2]));
}

__global__ void kernelFixTStatus_compact(TStatus* d_TStatus,
                                         int*     d_newIndex,
                                         int      numberofprocess,
                                         int      poffset)
{
...
__global__ void kernelFixSubseg2tri_compact(int* d_subseg2tri,
                                            int* d_newIndex,
                                            int  numberofprocess,
                                            int  poffset)
{
...
int compactTrianlgeList(IntD&     t_trianglelist,
                        IntD&     t_neighborlist,
                        TStatusD& t_TStatus,
                        IntD&     t_subseg2tri,
                        IntD&     t_valid,
                        ...
                        int       maxnumofblock)
{
    ...
    do {
        ...
        kernelMarkValidTriangles_compact<<<numberofblocks, BLOCK_SIZE>>>(
            thrust::raw_pointer_cast(&t_TStatus[0]),
            thrust::raw_pointer_cast(&t_valid[0]), numberofprocess, finishnum);
    ...
    thrust::exclusive_scan(t_valid.begin(), t_valid.begin() + numberoftriangles,
                           t_prefix.begin());

    int newnTris, lastitem, offset;
    cudaMemcpy(&newnTris,
               thrust::raw_pointer_cast(&t_prefix[0]) + numberoftriangles - 1,
               sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(&lastitem,
               thrust::raw_pointer_cast(&t_valid[0]) + numberoftriangles - 1,
               sizeof(int), cudaMemcpyDeviceToHost);
    newnTris += lastitem;
    ...
    cudaMemcpy(&offset, thrust::raw_pointer_cast(&t_prefix[0]) + newnTris,
               sizeof(int), cudaMemcpyDeviceToHost);
    ...
    do {
        ...
        kernelCollectTrianglesEmptySlots_compact<<<numberofblocks,
                                                   BLOCK_SIZE>>>(
            thrust::raw_pointer_cast(&t_TStatus[0]),
    ...
    do {
        ...
        kernelFixNeighbors_compact<<<numberofblocks, BLOCK_SIZE>>>(
            thrust::raw_pointer_cast(&t_neighborlist[0]),
            thrust::raw_pointer_cast(&t_prefix[0]), numberofprocess, finishnum);
...
__global__ void kernelMarkValidPoints_compact(PStatus* d_PStatus,
                                              int*     d_valid,
                                              int      numberofprocess,
                                              int      poffset)
{
...
__global__ void kernelCollectEmptyPointSlots_compact(PStatus* d_PStatus,
                                                     int*     d_prefix,
                                                     int*     d_empty,
                                                     int      numberofprocess,
                                                     int      poffset)
...
__global__ void kernelFillEmptyPointSlots_compact(PStatus* d_PStatus,
                                                  int*     d_prefix,
                                                  int*     d_empty,
                                                  REAL2*   d_pointlist,
                                                  int      numberofprocess,
...
__global__ void kernelUpdateTriangleList_compact(int* d_trianglelist,
                                                 int* d_prefix,
                                                 int* d_valid,
                                                 int  numberofprocess,
                                                 int  poffset,
...
__global__ void kernelFixPStatus_compact(PStatus* d_PStatus,
                                         int*     d_newIndex,
                                         int      numberofprocess,
                                         int      poffset)
{
...
int compactPointList(IntD&     t_trianglelist,
                     Real2D&   t_pointlist,
                     PStatusD& t_PStatus,
                     IntD&     t_valid,
                     IntD&     t_prefix,
                     ...
                     int       maxnumofblock)
{
    ...
    thrust::exclusive_scan(t_valid.begin(), t_valid.begin() + numberofpoints,
                           t_prefix.begin());

    int newnPoints, lastitem, offset;
    cudaMemcpy(&newnPoints,
               thrust::raw_pointer_cast(&t_prefix[0]) + numberofpoints - 1,
               sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(&lastitem,
               thrust::raw_pointer_cast(&t_valid[0]) + numberofpoints - 1,
               sizeof(int), cudaMemcpyDeviceToHost);
    newnPoints += lastitem;
    ...
    cudaMemcpy(&offset, thrust::raw_pointer_cast(&t_prefix[0]) + newnPoints,
               sizeof(int), cudaMemcpyDeviceToHost);
    ...
    do {
        ...
        kernelFillEmptyPointSlots_compact<<<numberofblocks, BLOCK_SIZE>>>(
            ...
            thrust::raw_pointer_cast(&t_prefix[0]),
            thrust::raw_pointer_cast(&t_valid[0]),
            thrust::raw_pointer_cast(&t_pointlist[0]), numberofprocess,
            finishnum, newnPoints, offset);
...
__global__ void kernelCompactSegment(int* d_trianglelist,
                                     int* d_subseg2tri,
                                     int* d_segmentlist,
                                     int  numberofprocess,
                                     int  poffset)
{
    ...
    int triIndex = decode_tri(otri);
    int triOri = decode_ori(otri);
    int p[] = {d_trianglelist[3 * triIndex + (triOri + 1) % 3],
...
void compactSegmentList(IntD& t_trianglelist,
                        IntD& t_subseg2tri,
                        IntD& t_segmentlist,
                        int   numberofsubsegs,
                        int   maxnumofblock)
{
    ...
    do {
        ...
        kernelCompactSegment<<<numberofblocks, BLOCK_SIZE>>>(
            thrust::raw_pointer_cast(&t_trianglelist[0]),
            thrust::raw_pointer_cast(&t_subseg2tri[0]),
            thrust::raw_pointer_cast(&t_segmentlist[0]), numberofprocess,
            finishnum);
...
__global__ void kernelUpdateNeighborsFormat2Int(int* d_neighborlist,
                                                int  numberofprocess,
                                                int  offset)
{
    int pos = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    int neighbors[3] = {d_neighborlist[3 * triIndex],
                        d_neighborlist[3 * triIndex + 1],
                        d_neighborlist[3 * triIndex + 2]};
...
void updateNeighborsFormat2Int(int* d_neighborlist,
                               int  numberoftriangles,
                               int  maxnumofblock)
{
    int numberofprocess;  // number of triangles processed per iteration
    ...
    do {
        ...
        kernelUpdateNeighborsFormat2Int<<<numberofblocks, BLOCK_SIZE>>>(
            d_neighborlist, numberofprocess, finishnum);
...
void GPU_Refine_Quality(triangulateio* input,
                        ...
                        TStatus**      ts_debug)
{
    ...
    Real2D t_pointlist((REAL2*)input->pointlist,
                       (REAL2*)input->pointlist + numberofpoints);
    IntD   t_trianglelist(input->trianglelist,
                        input->trianglelist + numberoftriangles * 3);
    IntD   t_neighborlist(input->neighborlist,
                        input->neighborlist + numberoftriangles * 3);
    IntD   t_segmentlist(input->segmentlist,
                       input->segmentlist + numberofsegments * 2);

    checkCUDAError("Read Input");

    // Transfer neighbor format to orientation triangles (otri)
    updateNeighborsFormat2Otri(t_trianglelist, t_neighborlist,
                               numberoftriangles, 5000);

    checkCUDAError("updateNeighborsFormat2Otri");

    // Memory variables
    int pointblock =
        numberofpoints * 0.5;      /* Number of points	allocated at once. */
    ...
    thrust::fill(t_PStatus.begin(), t_PStatus.begin() + numberofpoints,
                 inputPoint);
    thrust::fill(t_PStatus.begin() + numberofpoints, t_PStatus.end(),
                 emptyPoint);
    ...
    thrust::fill(t_TStatus.begin(), t_TStatus.begin() + numberoftriangles,
                 inputTri);
    thrust::fill(t_TStatus.begin() + numberoftriangles, t_TStatus.end(),
                 emptyTri);
    ...
    initSubsegs(t_segmentlist, t_subseg2tri, t_trianglelist, t_tri2subseg,
                last_triangle, last_subseg, 30, 5000);

    thrust::sequence(t_subseg2seg.begin(), t_subseg2seg.begin() + last_subseg,
                     0);  // Init subsegment to contain itself
    ...
    markAllEncsegs(t_pointlist, t_trianglelist, t_neighborlist, t_subseg2tri,
                   t_encmarker, last_subseg, run_mode, theta);
    ...
    if (false) {
        ...
        cudaMemcpy(debug_em, thrust::raw_pointer_cast(&t_encmarker[0]),
                   sizeof(int) * last_subseg, cudaMemcpyDeviceToHost);
        cudaMemcpy(debug_tl, thrust::raw_pointer_cast(&t_trianglelist[0]),
                   sizeof(int) * 3 * last_triangle, cudaMemcpyDeviceToHost);
        cudaMemcpy(debug_nl, thrust::raw_pointer_cast(&t_neighborlist[0]),
                   sizeof(int) * 3 * last_triangle, cudaMemcpyDeviceToHost);
        cudaMemcpy(debug_pl, thrust::raw_pointer_cast(&t_pointlist[0]),
                   sizeof(REAL2) * last_point, cudaMemcpyDeviceToHost);
        cudaMemcpy(debug_st, thrust::raw_pointer_cast(&t_subseg2tri[0]),
                   sizeof(int) * last_subseg, cudaMemcpyDeviceToHost);
        ...
        for (int i = 0; i < last_subseg; i++) {
            ...
            if (debug_em[i] == -1 && tag)
                printf(
                    "Segment %d: I am encroached but marked as "
                    "non-encroached\n",
                    i);

            if (debug_em[i] == 0 && !tag)
                printf(
                    "Segment %d: I am not encroached but marked as "
                    "encroached\n",
                    i);
        }
        printf("Finish Checking\n");
    ...
    splitEncsegs(t_pointlist, t_PStatus, t_trianglelist, t_neighborlist,
                 t_tri2subseg, t_TStatus, t_segmentlist, t_subseg2tri,
                 t_subseg2seg,
                 ...
                 t_list4,      // linklist
                 t_list5,      // linkslot
                 t_emptypoints, t_emptytriangles, pointblock, triblock,
                 &numberofemptypoints, &numberofemptytriangles, &last_point,
                 &last_triangle, &last_subseg, run_mode, theta, -1);
    ...
    if (false) {
        ...
        cudaMemcpy(debug_pl, thrust::raw_pointer_cast(&t_pointlist[0]),
                   sizeof(REAL2) * last_point, cudaMemcpyDeviceToHost);
        cudaMemcpy(debug_ts, thrust::raw_pointer_cast(&t_TStatus[0]),
                   sizeof(TStatus) * last_triangle, cudaMemcpyDeviceToHost);
    ...
    while (true) {
        ...
        numberofbad = updateActiveListToBadTriangles(
            t_pointlist, t_PStatus, t_trianglelist, t_neighborlist,
            t_segmentlist, t_subseg2seg, t_tri2subseg, t_TStatus, t_list0,
        ...
        splitTriangles(
            t_pointlist, t_PStatus, t_trianglelist, t_neighborlist,
            t_tri2subseg, t_TStatus, t_subseg2tri, t_encmarker,
            t_list0,    // encroached list
            ...
            t_list3,    // sinks
            t_list4,    // shortest
            t_trimarker, t_emptypoints, t_emptytriangles, pointblock, triblock,
            &numberofemptypoints, &numberofemptytriangles, &last_point,
            &last_triangle, &last_subseg, offconstant, offcenter, run_mode,
            theta, iteration);
        ...
        //	TStatus * debug_ts = new TStatus[last_triangle];
        //	cudaMemcpy(debug_tl,
        //thrust::raw_pointer_cast(&t_trianglelist[0]),sizeof(int)*3*last_triangle,cudaMemcpyDeviceToHost);
        //	cudaMemcpy(debug_pl,
        //thrust::raw_pointer_cast(&t_pointlist[0]),sizeof(REAL2)*last_point,cudaMemcpyDeviceToHost);
        //	cudaMemcpy(debug_ts,
        //thrust::raw_pointer_cast(&t_TStatus[0]),sizeof(TStatus)*last_triangle,
        //cudaMemcpyDeviceToHost); 	for(int i = 0; i < last_triangle; i++)
        //	{
        //		if(!debug_ts[i].isNull())
        //		{
        //			bool errorflag = false;
        ...
        //			}
        //			if(errorflag)
        //				printf("After splitTriangles - Tri %d: Duplicate
        //vertice\n",i);
        //		}
        //	}
        //}
        ...
        splitEncsegs(t_pointlist, t_PStatus, t_trianglelist, t_neighborlist,
                     ...
                     t_emptypoints, t_emptytriangles, pointblock, triblock,
                     &numberofemptypoints, &numberofemptytriangles, &last_point,
                     &last_triangle, &last_subseg, run_mode, theta, iteration);

        // cudaDeviceSynchronize();
        // sdkStopTimer( &inner_timer );
        // time_segs += sdkGetTimerValue( &inner_timer );

        updatePStatus2Old(t_PStatus, last_point);
        ...
        //			}
        //			if(errorflag)
        //				printf("After splitEncsegs - Tri %d: Duplicate
        //vertice\n",i);
        //		}
        //	}
        //}
    ...
    // triangles
    last_triangle = compactTrianlgeList(
        t_trianglelist, t_neighborlist, t_TStatus, t_subseg2tri, t_list0,
        t_list1, last_triangle, last_subseg, 5000);

    last_point =
        compactPointList(t_trianglelist, t_pointlist, t_PStatus, t_list0,
                         t_list1, last_triangle, last_point, 5000);

    compactSegmentList(t_trianglelist, t_subseg2tri, t_segmentlist, last_subseg,
                       5000);

    // points
    result->numberofpoints = last_point;
    result->pointlist = new double[2 * last_point];
    cudaMemcpy(result->pointlist, thrust::raw_pointer_cast(&t_pointlist[0]),
               2 * last_point * sizeof(double), cudaMemcpyDeviceToHost);

    // triangles
    result->numberoftriangles = last_triangle;
    result->trianglelist = new int[3 * last_triangle];
    cudaMemcpy(result->trianglelist,
               thrust::raw_pointer_cast(&t_trianglelist[0]),
               3 * last_triangle * sizeof(int), cudaMemcpyDeviceToHost);

    // neighbors
    updateNeighborsFormat2Int(thrust::raw_pointer_cast(&t_neighborlist[0]),
                              last_triangle, 5000);

    result->neighborlist = new int[3 * last_triangle];
    cudaMemcpy(result->neighborlist,
               thrust::raw_pointer_cast(&t_neighborlist[0]),
               3 * last_triangle * sizeof(int), cudaMemcpyDeviceToHost);
    ...
    cudaMemcpy(result->segmentlist, thrust::raw_pointer_cast(&t_segmentlist[0]),
               2 * last_subseg * sizeof(int), cudaMemcpyDeviceToHost);

    // debug
    if (ps_debug != NULL) {
        *ps_debug = new PStatus[last_point];
        cudaMemcpy(*ps_debug, thrust::raw_pointer_cast(&t_PStatus[0]),
                   last_point * sizeof(PStatus), cudaMemcpyDeviceToHost);
    }
    if (ts_debug != NULL) {
        *ts_debug = new TStatus[last_triangle];
        cudaMemcpy(*ts_debug, thrust::raw_pointer_cast(&t_TStatus[0]),
                   last_triangle * sizeof(TStatus), cudaMemcpyDeviceToHost);
    }
    t_pointlist.clear();
    t_segmentlist.clear();
