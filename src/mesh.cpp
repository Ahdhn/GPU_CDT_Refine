                         ...
                         double         min_input_angle)
{
    ...
    while (!otag && shift < numOfSegments) {
        ...
        if (min_input_angle >= 60.0)
            strs << "input/d" << distribution << "_s" << seed << "_p"
                 << numOfPoints << "_c" << (numOfSegments - shift);
        else
            strs << "input/d" << distribution << "_s" << seed << "_p"
                 << numOfPoints << "_c" << (numOfSegments - shift)
    ...
    if (otag)
        memcpy(inPointsCont.pointlist, oldInput.pointlist,
               sizeof(double) * 2 * numOfPoints);
    else {
    ...
    int convexsize =
        dtResult.numberofsegments;  // the number of convex hull segments
    ...
    if (otag) {
        ...
        inPointsCont.segmentlist =
            new int[numOfSegments < convexsize ? convexsize * 2 :
                                                 numOfSegments * 2];
        memcpy(inPointsCont.segmentlist, oldInput.segmentlist,
               2 * sizeof(int) * oldInput.numberofsegments);
        ...
        inPointsCont.segmentlist =
            new int[numOfSegments < convexsize ? convexsize * 2 :
                                                 numOfSegments * 2];
        memcpy(inPointsCont.segmentlist, dtResult.segmentlist,
               2 * sizeof(int) * convexsize);  // copy convex hull to input
    }
    ...
    while (num < numOfSegments - convexsize) {
        ...
        for (int i = 0; i < inPointsCont.numberofsegments; i++) {
            ...
            if ((p0 == index0 && p1 == index1) ||
                (p0 == index1 && p1 == index0)) {
                // this segment already exists
                pass = false;
                break;
            } else if (p0 == index0 || p0 == index1 || p1 == index0 ||
                       p1 == index1) {
                // continue; // dont need to check when want to generate input
                // with small angles
                ...
                // if( incidentL && incidentR) // this segment is incident to
                // more than 1 segments
                //{
                ...
                if (dotproduct < 0)  // obtuse angle
                    ...
                    REAL cossquare = dotproduct * dotproduct /
        ...
        if (!pass)  // fail to generate, try again
            ...
            inPointsCont.segmentlist[2 * inPointsCont.numberofsegments + 1] =
                p1;
...
void saveCDT(triangulateio* input, char* name)
{
    ...
    for (int i = 0; i < input->numberoftriangles; i++)
        fprintf(fp, "%d %d %d\n", input->trianglelist[3 * i],
                input->trianglelist[3 * i + 1], input->trianglelist[3 * i + 2]);
    for (int i = 0; i < input->numberoftriangles; i++)
        fprintf(fp, "%d %d %d\n", input->neighborlist[3 * i],
                input->neighborlist[3 * i + 1], input->neighborlist[3 * i + 2]);
...
bool readCDT(triangulateio* input, char* name)
{
    ...
    while (fgets(buf, 100, fp) != NULL) {
        if (ln == 0) {
            ...
            if (sscanf(buf, "%lf %lf", input->pointlist + 2 * (ln - 3),
            ...
            if (sscanf(
                ...
                return false;
                   ...
                   input->numberofpoints + 2 * input->numberoftriangles + 3) {
            if (sscanf(buf, "%d %d %d",
                       input->neighborlist + 3 * (ln - input->numberofpoints -
                                                  input->numberoftriangles - 3),
                       input->neighborlist +
                           3 * (ln - input->numberofpoints -
                                input->numberoftriangles - 3) +
                           1,
                       input->neighborlist +
                           3 * (ln - input->numberofpoints -
        ...
        } else if (ln < input->numberofpoints + 2 * input->numberoftriangles +
                            input->numberofsegments + 3) {
            if (sscanf(
                    buf, "%d %d",
                    input->segmentlist + 2 * (ln - input->numberofpoints -
                                              2 * input->numberoftriangles - 3),
                    input->segmentlist +
                        2 * (ln - input->numberofpoints -
                             2 * input->numberoftriangles - 3) +
                        1) != 2) {
                printf("Incorrect format\n");
                fclose(fp);
